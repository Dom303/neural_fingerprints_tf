...
#     parser.add_argument("eval_losses", default="RMSE", nargs='+', help="Loss functions to be used for evaluating the model. "
...
def run_neural_fingerprints(params):

    # If a configuration file is provided, load the configuration
    if params['paths']['config']:
        with open(params['paths']['config'], 'r') as f:
            config = json.load(f)
    else:
    ...
    config['model']['loss_fn'] = params['train_loss']
    # Add random seed to config dictionary
    config['train']['seed'] = params['seed']

    # Create NeuralFingerprints object for the training model
    model_tr = NeuralFingerprints(config=config, input_batches_file=params['paths']['train'], is_training=True)

    # Create NeuralFingerprints object for the validation model (if necessary)
    model_val = None
    if params['paths']['val']:
        model_val = NeuralFingerprints(config=config, input_batches_file=params['paths']['val'], is_training=False)
